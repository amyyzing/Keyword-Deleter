...
{
    public static SafeFileHandle OpenHandle(string path, bool backup)
    {
    ...
    public static FileStream OpenRead(string path, bool backup)
    {
        var handle = OpenHandle(path, backup);
        try
        {
            return new FileStream(handle, FileAccess.Read, bufferSize: 1, isAsync: true);
        }
        catch
        {
            handle.Dispose();
